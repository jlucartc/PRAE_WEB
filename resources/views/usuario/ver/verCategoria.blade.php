...
<div class="row justify-content-center">
  <div class="mt-5 pt-5 col-sm-10 pb-5">
            ...
            <div id="confirmarDelecao" class="modal fade">
              <div class="modal-dialog">
                <div class="modal-content">
                  ...
                  <div class="modal-footer">
                      <form action="{{ route('usuario.deletarCategoria',['id' => $categoria->id]) }}" method="POST">{{ csrf_field() }}<button class="btn btn-success mr-2" type="submit" name="button" title="confirmar mudanças">Confirmar</button></form><button class="btn btn-danger" type="button" data-dismiss="modal" name="button" title="cancelar mudanças">Cancelar</button>
                  </div>
                </div>
              </div>
            </div>
    ...
    <h2 class="mt-5 mb-3">Lista de descrições<a href="{{ route('usuario.adicionarDescricao',['id' => $categoria->id]) }}" class="btn btn-secondary mr-2 float-right" >Adicionar descrição  <span class="fas fa-plus"></span> </a></h2>
    ...
    @endif
    <h2 class="mt-5 mb-3">Lista de itens<a href="{{ route('usuario.adicionarItem',['id' => $categoria->id]) }}" class="btn btn-secondary mr-2 float-right" >Adicionar item  <span class="fas fa-plus"></span> </a></h2>
    @if( !$itens->isEmpty() )
      <div class="list-group">
        @foreach( $itens as $item )
          <a href="{{ route('usuario.verItem',['id' => $item->id]) }}" class="list-group-item list-group-item-action border border-dark">{{ ucfirst($item->nome) }}</a>
        @endforeach
      </div>
    @else
      <div class="alert alert-info">
    ...
    @endif
    <h2 class="mt-5 mb-3">Lista de documentos<a href="{{ route('usuario.adicionarDocumento',['id' => $categoria->id]) }}" class="btn btn-secondary mr-2 float-right" >Adicionar documento  <span class="fas fa-plus"></span> </a></h2>

...
<div class="row justify-content-center">
  <div class="mt-5 pt-5 col-sm-10">
    <div class="card">
      ...
      <div class="card-body">
        <form class="form" action="{{ route('sistema.salvarDivisao',['id' => $divisao->id]) }}" method="post">
            {{ csrf_field() }}
            Nome
      ...
      </form><a href="{{ route('sistema.verCoordenadoria',['id' => $divisao->coordenadoria_id ]) }}" class="btn btn-primary mr-2" >Cancelar</a><button class="btn btn-danger" type="button" data-toggle="modal" data-target="#confirmarDelecaoDivisao" name="button">Apagar divisão</button>
        <div class="modal fade" id="confirmarDelecaoDivisao">
          <div class="modal-dialog">
            <div class="modal-content">
              ...
              <div class="modal-footer">
                <form class="form" action="{{ route('sistema.deletarDivisao',['id' => $divisao->id]) }}" method="post">
                  {{ csrf_field() }}
                  <button class="btn btn-success mr-2" type="submit" name="button">Confirmar</button><button class="btn btn-danger" data-dismiss="modal" type="button" name="button">Cancelar</button>
                </form>
              </div>

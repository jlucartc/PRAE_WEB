...
<div class="row justify-content-center">
  <div class="mt-5 pt-5 col-sm-10 pb-5">
    <div class="card mb-5">
        ...
        <div class="card-body">
            <form class="form" action="{{ route('sistema.salvarCoordenadoria',['id' => $coordenadoria->id]) }}" method="post">
                {{ csrf_field() }}
                Nome
        ...
        <div class="card-footer">
            <button class="btn btn-success mr-2" type="button" data-toggle="modal" data-target="#confirmarMudancas" name="button">Salvar mudanças</button><a href="{{ route('sistema.coordenadorias') }}" class="btn btn-primary mr-2" >Cancelar</a><button class="btn btn-danger" data-toggle="modal" data-target="#confirmarDelecaoCoordenadoria" type="button" name="button">Apagar coordenadoria</button>
            ...
            <div id="confirmarDelecaoCoordenadoria" class="modal fade">
              <div class="modal-dialog">
                <div class="modal-content">
                  ...
                  <div class="modal-footer">
                      <form class="form" action="{{ route('sistema.deletarCoordenadoria',['id' => $coordenadoria->id]) }}" method="post"> {{ csrf_field() }} <button class="btn btn-success mr-2" type="submit" >Confirmar</button> </form><button class="btn btn-danger" type="button" data-dismiss="modal" name="button">Cancelar</button>
                  </div>
                </div>
              </div>
            </div>
    ...
    <h2 class="mt-5 mb-3">Lista de compromissos<a href="{{ route('sistema.adicionarCompromisso',['id' => $coordenadoria->id]) }}" class="btn btn-secondary mr-2 float-right" >Adicionar compromisso  <span class="fas fa-plus"></span> </a></h2>
    @if( !$compromissos->isEmpty() )
      <div class="list-group">
        @foreach( $compromissos as $compromisso )
          <a href="{{ route('sistema.verCompromisso',['id' => $compromisso->id]) }}" class="list-group-item list-group-item-action border border-dark">{{ ucfirst($compromisso->titulo) }} - {{ $compromisso->data }}</a>
        @endforeach
      </div>
    @else
    ...
    @endif
    <h2 class="mt-5 mb-3 mt-3">Lista de divisões<a href="{{ route('sistema.adicionarDivisao',['id' => $coordenadoria->id]) }}" class="btn btn-secondary mr-2 float-right" >Adicionar divisão  <span class="fas fa-plus"></span> </a></h2>
    @if( !$divisoes->isEmpty() )
      <div class="list-group">
        @foreach( $divisoes as $divisao )
          <a href="{{ route('sistema.verDivisao',['id' => $divisao->id]) }}" class="list-group-item list-group-item-action border border-dark">{{ ucfirst($divisao->nome) }}</a>
        @endforeach
      </div>
    @else
      <div class="alert alert-info">
    ...
    @endif
    <h2 class="mt-5 mb-3 mt-3">Lista de mapas<a href="{{ route('sistema.adicionarMapa',['id' => $coordenadoria->id]) }}" class="btn btn-secondary mr-2 float-right" >Adicionar mapa  <span class="fas fa-plus"></span> </a></h2>
    @if( !$mapas->isEmpty() )
      <div class="list-group">
